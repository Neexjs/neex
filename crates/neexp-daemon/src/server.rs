...
//! Note: Only available on Unix systems (Linux, macOS)
...
// Unix-only server implementation
#[cfg(unix)]
mod unix_impl {
    use super::*;
    use std::path::Path;
    ...
    impl DaemonServer {
        ...
        pub fn new(root: impl AsRef<Path>) -> Result<Self> {
            ...
            let socket_path = root.join(".neexp").join("daemon.sock");
...
#[cfg(unix)]
pub use unix_impl::DaemonServer;
...
#[cfg(windows)]
impl DaemonServer {
    pub fn new<P: AsRef<std::path::Path>>(_root: P) -> Result<Self> {
        Ok(Self)
    }

    pub async fn start(&mut self) -> Result<()> {
        tracing::warn!("Daemon mode is not supported on Windows");
        Ok(())
    }
}
