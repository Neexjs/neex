...
use neexp_daemon::{DaemonRequest, DaemonResponse};
...
async fn run_task(cwd: &PathBuf, task: &str) -> Result<()> {
    ...
    if let Ok(Some(cloud)) = CloudCache::try_new() {
        if let Ok(Some(data)) = cloud.download(&key).await {
            if let Ok(output) = serde_json::from_slice::<neexp_core::TaskOutput>(&data) {
...
async fn run_symbols(cwd: &PathBuf, task: &str) -> Result<()> {
    ...
    // Load previous cache
    let cache_path = cwd.join(".neexp").join("symbols.json");
...
async fn run_all(
    ...
    if use_raw {
        ...
        let ok = results
            .iter()
            .filter(|r| r.status == neexp_core::TaskStatus::Completed)
            .count();
        let fail = results
            .iter()
            .filter(|r| r.status == neexp_core::TaskStatus::Failed)
            .count();
...
async fn create_tasks(
    cwd: &Path,
    order: &[&neexp_core::WorkspaceNode],
    task: &str,
    graph: &DepGraph,
) -> Vec<SchedulerTask> {
...
async fn show_info(cwd: &PathBuf) -> Result<()> {
    ...
    // Cache
    let cache = cwd.join(".neexp").join("cache");
    if cache.exists() {
...
fn hash_file(file: &PathBuf) -> Result<()> {
    ...
    let hash = if is_parseable(file) {
        ...
        neexp_core::ast_hasher::hash_raw(&content)?
    };
    println!("{}", hash);
    Ok(())
}
