...
import { StartManager } from '../start-manager';
  ...
  program
    ...
    .action(async (file, options) => {
      try {
              ...
              if (fs.existsSync(alternativeFile)) {
                ...
                for (const location of commonLocations) {
                  ...
                  if (fs.existsSync(testPath)) {
                    ...
                    if (options.verbose) {
                      loggerManager.printLine(
                        `Found application file: ${location}`,
                        'info'
                      );
                    }
                    break;
                ...
                if (!found) {
                  throw new Error(
                    `Application file not found. Tried: ${targetFile}, ${mainFile}, and common locations.`
                  );
                }
              }
            } catch (parseError) {
              throw new Error(
                `Failed to parse package.json: ${(parseError as Error).message}`
              );
            }
          } else {
        ...
        if (options.verbose) {
          loggerManager.printLine(
            `File: ${path.relative(process.cwd(), resolvedFile)}`
          );
          ...
          if (healthCheck) {
            loggerManager.printLine(
              `Health Check: http://localhost:${options.healthPort}/health`
            );
          }
        }
        ...
        if (error instanceof Error) {
          loggerManager.printLine(
            `${chalk.red(figures.cross)} ${error.message}`,
            'error'
          );
        } else {
          loggerManager.printLine(
            `${chalk.red(figures.cross)} Startup failed`,
            'error'
          );
        }
        process.exit(1);
