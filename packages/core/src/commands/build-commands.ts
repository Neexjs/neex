...
export function addBuildCommands(program: Command): {
  cleanupBuild: () => void;
} {
      ...
      try {
        ...
        if (!options.quiet) {
          loggerManager.printLine(
            `${chalk.green(figures.play)} Building TypeScript project from ${chalk.cyan(sourceDir)}`,
            'info'
          );
        }
        ...
        if (!options.watch && !options.quiet) {
          loggerManager.printLine(
            `${chalk.green(figures.tick)} Build completed successfully`,
            'info'
          );
        }
      } catch (error: unknown) {
        if (error instanceof Error) {
          loggerManager.printLine(
            `${chalk.red(figures.cross)} Build failed: ${error.message}`,
            'error'
          );
        } else {
          loggerManager.printLine(
            `${chalk.red(figures.cross)} An unknown build error occurred`,
            'error'
          );
        }
        process.exit(1);
  ...
  program
    ...
    .action(async source => {
      try {
        ...
        loggerManager.printLine(
          `${chalk.blue(figures.info)} Compiling TypeScript...`,
          'info'
        );
        ...
        await buildManager.build();
        loggerManager.printLine(
          `${chalk.green(figures.tick)} Compilation completed`,
          'info'
        );
      } catch (error: unknown) {
        if (error instanceof Error) {
          loggerManager.printLine(
            `${chalk.red(figures.cross)} Compilation failed: ${error.message}`,
            'error'
          );
        } else {
          loggerManager.printLine(
            `${chalk.red(figures.cross)} An unknown compilation error occurred`,
            'error'
          );
        }
        process.exit(1);
