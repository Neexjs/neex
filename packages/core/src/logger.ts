...
class Logger {
  ...
  private commandColors: Map<string, typeof chalk> = new Map();
  ...
  private generateColor(command: string): typeof chalk {
  ...
  startSpinner(command: string): void {
    ...
    const interval = setInterval(() => {
      ...
      process.stdout.write(
        `\r${prefix} ${color(frame)} ${chalk.dim('Running...')}`
      );
    }, 80);
  ...
  printSuccess(result: RunResult): void {
    ...
    console.log(
      `${prefix} ${chalk.green(figures.tick)} ${chalk.green('Completed')}${durationStr}`
    );
  }
  ...
  printError(result: RunResult): void {
    ...
    console.error(
      `${prefix} ${chalk.red(figures.cross)} ${chalk.red('Failed')}${errorCode}${durationStr}`
    );
  ...
  printEnd(result: RunResult, minimalOutput: boolean): void {
    ...
    if (minimalOutput) {
      if (!result.success) {
        const status =
          result.code !== null ? `failed (code ${result.code})` : 'failed';
        this.printLine(
          `${prefix} ${chalk.red(figures.cross)} ${result.command} ${status} ${duration}`,
          'error'
        );
      }
    } else {
      if (result.success) {
        this.printLine(
          `${prefix} ${chalk.green(figures.tick)} Command "${result.command}" finished successfully ${duration}`,
          'info'
        );
      } else {
        ...
        this.printLine(
          `${prefix} ${chalk.red(figures.cross)} Command "${result.command}" failed${errorCode}${errorMessage} ${duration}`,
          'error'
        );
      }
    }
  ...
  printSummary(results: RunResult[]): void {
    ...
    console.log(
      `${chalk.green(`${figures.tick} ${successful} succeeded`)}, ${chalk.red(`${figures.cross} ${failed} failed`)}`
    );
    console.log(
      `${chalk.blue(figures.info)} ${chalk.dim(`Total execution time: ${totalSeconds}s`)}`
    );
      ...
      results
        ...
        .forEach(result => {
          ...
          const duration = result.duration
            ...
            : '';
          console.log(
            `  ${chalk.green(figures.tick)} ${color(result.command)}${duration}`
          );
        });
    }
      ...
      results
        ...
        .forEach(result => {
          ...
          const duration = result.duration
            ...
            : '';
          const code =
            result.code !== null ? chalk.red(` [code: ${result.code}]`) : '';
          console.log(
            `  ${chalk.red(figures.cross)} ${color(result.command)}${code}${duration}`
          );
        });
    }
  }
