...
export function addDevCommands(program: Command): { cleanupDev: () => void } {
        ...
        .action(async (file, options) => {
            try {
                ...
                const cleanupAndExit = () => {
                    ...
                    if (devManager) {
                        loggerManager.printLine(`\n${chalk.yellow('⏹')} Received SIGINT, shutting down...`, 'info');
                ...
                if (error instanceof Error) {
                    loggerManager.printLine(`${chalk.red('✖')} ${error.message}`, 'error');
                } else {
                    loggerManager.printLine(`${chalk.red('✖')} Unknown error occurred`, 'error');
                }
                process.exit(1);
            }
        });
    ...
    program
        ...
        .action(() => {
            ...
            if (cleaned) {
                loggerManager.printLine(`${chalk.green('✓')} Cache cleaned successfully`, 'info');
            } else {
                loggerManager.printLine(`${chalk.blue('ℹ')} No cache to clean`, 'info');
            }
        });
    ...
    program
        ...
        .action((options) => {
            ...
            if (!fs.existsSync(configPath)) {
                loggerManager.printLine(`${chalk.red('✖')} TypeScript config not found: ${configPath}`, 'error');
                process.exit(1);
            }
            ...
            try {
                ...
                loggerManager.printLine(`${chalk.green('✓')} TypeScript config is valid`, 'info');
                ...
                if (config.compilerOptions && !options.quiet) {
                    ...
                    loggerManager.printLine(`  Module: ${opts.module || 'CommonJS'}`, 'info');
                    loggerManager.printLine(`  Strict: ${opts.strict || false}`, 'info');
                    loggerManager.printLine(`  Source Maps: ${opts.sourceMap || false}`, 'info');
                    loggerManager.printLine(`  Skip Lib Check: ${opts.skipLibCheck || false}`, 'info');
                }
                
            } catch (error) {
                loggerManager.printLine(`${chalk.red('✖')} Invalid TypeScript config: ${(error as Error).message}`, 'error');
                process.exit(1);
            }
        });
    ...
    program
        ...
        .action(() => {
            ...
            console.log('');
            loggerManager.printLine(`${chalk.blue('⚡')} ${chalk.bold('neex dev')} - Development Server Info`, 'info');
            console.log('');
            
            loggerManager.printLine(`${chalk.dim('System:')}`, 'info');
            loggerManager.printLine(`  Platform: ${os.platform()} ${os.arch()}`, 'info');
            loggerManager.printLine(`  Node.js: ${process.version}`, 'info');
            loggerManager.printLine(`  Memory: ${Math.round(process.memoryUsage().heapUsed / 1024 / 1024)}MB used`, 'info');
            
            console.log('');
            loggerManager.printLine(`${chalk.dim('Features:')}`, 'info');
            loggerManager.printLine(`  ${chalk.green('✓')} Ultra-fast TypeScript compilation`, 'info');
            loggerManager.printLine(`  ${chalk.green('✓')} Intelligent module caching`, 'info');
            loggerManager.printLine(`  ${chalk.green('✓')} Hot reload with dependency tracking`, 'info');
            loggerManager.printLine(`  ${chalk.green('✓')} Source map support`, 'info');
            loggerManager.printLine(`  ${chalk.green('✓')} Memory-optimized processing`, 'info');
            ...
            loggerManager.printLine(`  TypeScript Config: ${tsConfigExists ? chalk.green('✓') : chalk.red('✖')}`, 'info');
            loggerManager.printLine(`  Package.json: ${packageJsonExists ? chalk.green('✓') : chalk.red('✖')}`, 'info');
