...
export function addServerCommands(program: Command): void {
    ...
    program
        ...
        .action(async (commands, options) => {
            try {
                console.log(chalk.blue(`${figures.info} Starting servers in parallel mode...`));
                ...
                if (error instanceof Error) {
                    console.error(chalk.red(`${figures.cross} Server Error: ${error.message}`));
                } else {
                    console.error(chalk.red(`${figures.cross} An unknown server error occurred`));
                }
                process.exit(1);
            }
        });
