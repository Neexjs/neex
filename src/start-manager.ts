...
export class StartManager {
    ...
    constructor(options: StartOptions) {
        ...
        this.debouncedRestart = debounce(this.restartAll.bind(this), options.restartDelay);
    }
        ...
        if (this.options.envFile && fs.existsSync(this.options.envFile)) {
            try {
                ...
                if (this.options.verbose) {
                    this.log(`Failed to load environment file: ${(error as Error).message}`, 'warn');
                }
            }
        }
    }
    ...
    private async startCluster(): Promise<void> {
        ...
        // Multi-worker mode
        this.log(`${chalk.blue(figures.play)} Starting production server (${this.options.workers} workers)`);
        ...
        try {
            ...
            this.log(`${chalk.green(figures.tick)} Server ready on port ${this.options.port || 8000} (${this.workers.size} workers)`);
        } catch (error) {
            this.log(`Failed to start some workers: ${(error as Error).message}`, 'error');
            if (this.workers.size > 0) {
                this.log(`${chalk.yellow(figures.warning)} Server partially ready on port ${this.options.port || 8000} (${this.workers.size} workers)`);
            }
        }
    }
    ...
    private setupWatcher(): void {
        ...
        this.watcher.on('change', (filePath: string) => {
            if (this.options.verbose) {
                this.log(`File changed: ${path.relative(this.options.workingDir, filePath)}`);
            }
