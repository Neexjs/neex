...
import { displayNeexpLogo } from "./utils/logo.js";
...
async function main(): Promise<void> {
  console.clear();
  displayNeexpLogo(version);

  const program = new Command()
    .name("create-neexp")
    .description("Create a new Neexp monorepo with one command")
  ...
  p.intro(pc.bgCyan(pc.black(" Welcome to Neexp! ")));
  ...
  try {
    ...
    if (!inputProjectName) {
      const nameResult = await p.text({
        ...
        placeholder: "my-neexp-app",
        defaultValue: "my-neexp-app",
    ...
    // Success message
    console.log();
    console.log(pc.bold(pc.green("🚀 Neexp project created successfully!")));
    console.log();
    ...
    console.log(`   ${pc.cyan("neexp dev --all")}    ${pc.dim("# Start all apps")}`);
    console.log();
